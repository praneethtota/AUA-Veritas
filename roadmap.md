...
**6.8 — Rolling backups, not one-shot**
The 70% threshold triggers the first backup, but backups continue rolling for as long as the conversation lives. After a model is refreshed on a new thread, a fresh token counter starts for that model's new thread. When that thread also hits 70%, a new backup is generated and stored as a new row with a later timestamp. The router always injects the most recent backup for each model.

Additionally, a periodic backup runs every 20 messages regardless of token count. This keeps the backup fresh throughout a long conversation rather than relying solely on the context-limit trigger. A model on its third refresh thread, 200 messages in, will have a backup that reflects those 200 messages — not just the original handoff summary.

The `context_backups` table is append-only. Old rows are never deleted — they form an audit trail of how the context evolved. Only the most recent row per `(model_id, conversation_id)` is injected.

- Done condition: A user can work in a long conversation, step away for a day, come back, and the app continues seamlessly in the same window with all prior context intact. The user never has to paste a restart prompt manually. If the conversation runs for 1,000 messages across multiple context refreshes, the injected backup at any point reflects the most recent 20-message snapshot, not the original session summary.
